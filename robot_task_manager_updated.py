...
"""
Robot Task Manager - File Output Version

This program simulates a robot completing multiple target-point tasks.
It supports task priority, task status statistics, and file output.

机器人任务管理器 - 文件输出版本

这个程序模拟一个机器人完成多个目标点任务。
它支持任务优先级、任务状态统计，并可以把最终结果保存到文件中。
"""
...
# 输出报告文件名
report_file_name = "task_report.txt"
...
def generate_report_lines(robot_name, task_list, final_battery):
    """
    Generate report content as a list of text lines.
    ...
    report_lines = []          #创建一个空列表，用来保存报告内容

    report_lines.append("Robot Task Report")      #往报告里添加一行：Robot Task Report
    report_lines.append("==============================")
    report_lines.append(f"Robot name: {robot_name}")
    report_lines.append(f"Total tasks: {total_tasks}")
    report_lines.append(f"Completed tasks: {completed_tasks}")
    report_lines.append(f"Unfinished tasks: {unfinished_tasks}")
    report_lines.append(f"Final battery: {final_battery}%")
    report_lines.append(f"Task completion rate: {completion_rate:.1f}%")  #完成率保留 1 位小数
    report_lines.append("")
...
def save_report_to_file(file_name, report_lines):
    """
    ...
    print(f"Task report saved to {file_name}")
...
# ==============================
# 4. Final Summary and File Output
# 4. 最终总结和文件输出
# ==============================

print_summary(sorted_tasks, battery)

report_lines = generate_report_lines(robot_name, sorted_tasks, battery)
save_report_to_file(report_file_name, report_lines)